...
class DoReturnRateTag(object):
    ...
    def start():
        ...
        # 取四级标签的 rule
        rule_row = df.query("level == 4 and id == 240").iloc[0]
        rule = rule_row['rule'].split(";")
        selectTable = rule[0].split("=")[1]
        selectField = rule[1].split("=")[1].split("|")

        # 取五级标签
        attr = df.query("level == 5 and pid == 240")[['name', 'rule']]
        ...
        # 将退货状态的改为1，其他变成0
        df2['orderStatus'] = df2['orderStatus'].apply(lambda x: 1 if x == 0 else 0)

        # 按照用户ID分组，获取退货订单数量
        count_return_df = df2.groupby('memberId')['orderStatus'].sum().reset_index()
        count_return_df.columns = ['userId', 'returnRate']
