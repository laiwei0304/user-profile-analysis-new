...
class DoJobTag(object):
    ...
    def start():
        ...
        rule_row = df.query("level == 4 and id == 113").iloc[0]
        rule = rule_row['rule']
        if not rule:
        ...
        attr = df.query("level == 5 and pid == 113")[["name", "rule"]]
        ...
        # 打标签（不同模型不一样）
        rst = df2.merge(attr, left_on="job", right_on="rule", how="left") \
            .drop(columns=["job", "rule"]) \
            .rename(columns={"name": "job", "id": "userId"}) \
            .sort_values(by="userId")

        # 存储打好标签的数据
        rst.to_sql('tbl_job_tag', con=engine, if_exists='replace', index=False)
        print("职业标签计算完成！")
