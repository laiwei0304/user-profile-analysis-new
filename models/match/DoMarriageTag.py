...
class DoMarriageTag(object):
    ...
    def start():
        ...
        rule_row = df.query("level == 4 and id == 120").iloc[0]
        rule = rule_row['rule']
        if not rule:
        ...
        attr = df.query("level == 5 and pid == 120")[["name", "rule"]]

        # 从selectTable中提取selectField字段
        df2 = pd.read_sql(f'SELECT {", ".join(selectField)} FROM {selectTable}', con=engine)

        # 转换marriage和rule列为字符串类型
        df2['marriage'] = df2['marriage'].astype(str)
        attr['rule'] = attr['rule'].astype(str)

        # 打标签（不同模型不一样）
        rst = df2.merge(attr, left_on="marriage", right_on="rule", how="left") \
            .drop(columns=["marriage", "rule"]) \
            .rename(columns={"name": "marriage", "id": "userId"}) \
            .sort_values(by="userId")

        # 存储打好标签的数据
        rst.to_sql('tbl_marriage_tag', con=engine, if_exists='replace', index=False)
        print("婚姻状况标签计算完成！")
